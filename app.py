...
def predict():
    if model is None:
        return jsonify({
            "error": "النموذج غير متاح حالياً. يرجى المحاولة لاحقاً.",
            "success": False
        }), 503
    ...
    if file_extension not in allowed_extensions:
        return jsonify({
            "error": "نوع الملف غير مدعوم. يرجى استخدام صور بصيغة JPG, PNG, GIF",
            "success": False
        }), 400
    ...
    try:
        file.save(filepath)
        logger.info(f"📁 الصورة محفوظة في: {filepath}")
        ...
        try:
            ...
            logger.warning(f"تعذر حذف الملف: {cleanup_error}")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 بدء تشغيل خادم تصنيف الصور...")
    logger.info(f"📱 الرابط: http://0.0.0.0:{port}")
